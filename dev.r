...
# USArrests dataset
summary(lm(Murder ~ Assault * UrbanPop * Rape, data = USArrests))
summary(lm.gat(Murder ~ Assault * UrbanPop * Rape, data = USArrests, iterations = 6, penalty = 1e-9)$fit)

# swiss dataset
summary(lm(Fertility ~ Agriculture * Examination * Education, data = swiss))
summary(lm.gat(Fertility ~ Agriculture * Examination * Education, data = swiss, iterations = 6, penalty = 1e-9)$fit)

# trees dataset
summary(lm(Volume ~ Girth * Height, data = trees))
summary(lm.gat(Volume ~ Girth * Height, data = trees, iterations = 6, penalty = 1e-9)$fit)

# airquality dataset (omit NAs)
summary(lm(Ozone ~ Solar.R * Wind * Temp, data = na.omit(airquality)))
summary(lm.gat(Ozone ~ Solar.R * Wind * Temp, data = na.omit(airquality), iterations = 6, penalty = 1e-9)$fit)

# faithful dataset (simple nonlinear model)
summary(lm(eruptions ~ waiting, data = faithful))
summary(lm.gat(eruptions ~ waiting, data = faithful, iterations = 6, penalty = 1e-9)$fit)

# CO2 dataset (numeric subset only)
summary(lm(uptake ~ conc, data = subset(CO2, select = c(conc, uptake))))
summary(lm.gat(uptake ~ conc, data = subset(CO2, select = c(conc, uptake)), iterations = 6, penalty = 1e-9)$fit)
